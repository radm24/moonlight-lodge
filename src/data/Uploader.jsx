...
async function deleteCabins() {
  const { cabinsError } = await supabase.from("cabins").delete().gt("id", 0);
  if (cabinsError) return console.log(cabinsError.message);

  // Delete all cabin images except reserved ones
  const { data: images, listImagesError } = await supabase.storage
    .from("cabin-images")
    .list("");
  if (listImagesError) return console.log(listImagesError.message);

  const filteredImages = images
    .filter(
      (image) =>
        !RESERVED_CABIN_IMAGES.find((resImage) => resImage === image.name)
    )
    .map((image) => image.name);

  const { error: deleteImagesError } = await supabase.storage
    .from("cabin-images")
    .remove(filteredImages);
  if (deleteImagesError) console.log(deleteImagesError.message);
}
